# Contributing to auto-cad-cli

*English | [中文](CONTRIBUTING_zh.md)*

Thanks for improving **auto-cad-cli** — Autodesk AutoCAD drawing automation. This document covers building, testing, and submitting changes.
...
```bash
# Clone
git clone https://github.com/fatecannotbealtered/auto-cad-cli.git
cd auto-cad-cli

# --- Go variant ---
go mod download
make build                      # or: go build -o bin/auto-cad-cli ./auto_cad_cli
go test -race ./...
./bin/auto-cad-cli --help
...
Release standard: **Functional Contract Coverage = 100%**. Every public behavior documented in `README`, `SKILL`, reference pages, `auto-cad-cli reference`, `--help`, `context`, `doctor`, `changelog`, or `update` must have automated command-level tests.
...
Keep `auto-cad-cli reference` `release_readiness` and `auto-cad-cli doctor`'s `release_readiness` check honest when test evidence changes.
...
5. **SKILL** — add a `skills/auto-cad-cli/reference/<domain>.md` page and link it from `skills/auto-cad-cli/SKILL.md` (keep SKILL.md a short progressive-disclosure index).
6. **Docs** — update `README.md` / `README_zh.md` command lists and add a line under `## [Unreleased]` in `CHANGELOG.md`.

`reference` walks the command tree automatically, so new commands appear in `auto-cad-cli reference` without extra wiring.
