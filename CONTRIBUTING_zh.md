...
感谢改进 **auto-cad-cli** —— Autodesk AutoCAD 图纸自动化。本文档介绍如何构建、测试和提交改动。
...
# --- Go 变体 ---
go mod download
make build                      # 或：go build -o bin/auto-cad-cli ./auto_cad_cli
go test -race ./...
...
发布标准：**Functional Contract Coverage = 100%**。`README`、`SKILL`、reference 页面、`auto-cad-cli reference`、`--help`、`context`、`doctor`、`changelog` 或 `update` 中记录的每个公开行为，都必须有自动化命令级测试。
...
当测试证据变化时，同步保持 `auto-cad-cli reference` 的 `release_readiness` 和 `auto-cad-cli doctor` 的 `release_readiness` 检查真实可信。
...
5. **SKILL** —— 新增 `skills/auto-cad-cli/reference/<domain>.md` 页面，并从 `skills/auto-cad-cli/SKILL.md` 链接过去（SKILL.md 保持为简短的渐进式披露索引）。
6. **Docs** —— 更新 `README.md` / `README_zh.md` 命令列表，并在 `CHANGELOG.md` 的 `## [Unreleased]` 下加一行。

`reference` 会自动遍历命令树，新命令无需额外接线即出现在 `auto-cad-cli reference` 中。
