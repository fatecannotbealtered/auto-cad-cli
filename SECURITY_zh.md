...
**auto-cad-cli**（@fateforge/auto-cad-cli）的安全策略 —— Autodesk AutoCAD 图纸自动化。

## 支持的版本

安全修复只应用于默认分支上的**最新 minor 版本**，旧 minor 不做回移植。发布二进制通过 GitHub Releases（`fatecannotbealtered/auto-cad-cli`）和 npm 包 `@fateforge/auto-cad-cli` 分发。
...
- **GitHub 私有 advisory** —— 在 `https://github.com/fatecannotbealtered/auto-cad-cli/security/advisories/new` 创建草稿 advisory。
...
- **存储位置**：凭证只存在 `~/.auto-cad-cli/` 下（如 `config.json`、`profiles.json`）。
...
- **环境变量优先**：环境变量（如 `AUTO_CAD_CLI_HOST`、`AUTO_CAD_CLI_TOKEN`）优先于配置文件。在 CI / Agent 流程中优先用环境变量，避免把凭证落盘。
...
- **自更新同步 Skill**：裸 `update`（单命令、无 confirm token）成功后应同步整个内置 `skills/auto-cad-cli/` 目录，或返回等价于 `npx skills add fatecannotbealtered/auto-cad-cli -y -g` 的 `skill_sync_command`。
...
把 `auto-cad-cli` 接入自动化或 AI Agent 流程前，请先审阅这些假设。
